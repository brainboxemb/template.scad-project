...
These are copied from `tool.scad-project` v0.4.4. The updater scripts are
Python-free.

The template no longer maintains separate `update-externals.*` scripts; generic
dependency updating belongs to `update-repo.*`.

## Direct-only checkout rule

The template's normal bootstrap/update/build flow initializes only the direct
dependencies declared by the template itself.

Do not recursively initialize `lib.scad.clamps` development submodules.
Recursive traversal is only for an explicit full-tree integration test.
